...
if (isset($_POST['action']) && ($_POST['action'] == 'Submit')) {
  ...
  while ($row2 = mysqli_fetch_array($result2)) {
	...
	while($row3 = mysqli_fetch_array($result3)) {
	  $sql = "select * from student_fees_{$_SESSION['sessid']} where session_id={$row2['sid']}
          and term_id={$row3['id']} and class_id={$row2['cid']} and admission_number='$admission_number'";
      if (mysqli_num_rows(mysqli_query($con, $sql)) > 0) {
        $found = True;
        $url = "student_fees_report.php?action=Generate&admission_number={$admission_number}&session_id1={$row2['sid']}&term_id1={$row3['id']}&class_id1={$row2['cid']}";
		echo "Financial record is available for <a href='$url'> {$row2['sname']} Session {$row3['name']} Term, Class {$row2['cname']} </a><br><br>";
      }
	}
...
choose_student_ui("Financial History", "financial_history.php", $con);
?>