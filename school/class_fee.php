 ...
 (($_REQUEST['action'] == 'Generate') || ($_REQUEST['action'] == 'Print'))) {
  if ($_REQUEST['class_id'] == 'all_classes') {
    ...
    $sql="SELECT c.type, c.name, s.admission_number, s.firstname, 
     s.lastname, s.class_id, s.scholarship FROM student_fee sf
     JOIN (class c, student s) ON ( s.class_id = c.id
  ...
  if ($_REQUEST['action'] == 'Generate') {
   ...
   if ($_REQUEST['class_id'] == 'all_classes') {
     ...
     $sql="SELECT s.id, c.type, c.name, s.admission_number, s.firstname, 
      s.lastname, s.class_id, s.scholarship FROM student s
   ...
   while ($row = mysqli_fetch_array($result)) {
     ...
     if ($_REQUEST['action'] == 'Generate') {
       echo "
               <td>
                <a href='student_fee.php?class_id={$row['class_id']}&term_id={$_REQUEST['term_id']}&student_id={$row['id']}&action=Generate'>
     ...
     echo "
               ...
               <td>{$row['lastname']}</td>
	  ";
     echo "<td>" . get_value('class', 'name', 'id', $row['class_id'], 
      $con) . "</td>";
     echo "</tr></table></td>";
	...
	and student_id={$row['id']} order by date_of_payment";
        ...
        $sql1="SELECT sf.amount, c.type, c.name, s.admission_number, 
         ...
         s.class_id FROM student_fee sf
	 JOIN (class c, student s) ON ( s.class_id = c.id
     ...
     $result1 = mysqli_query($con, $sql1) or die(mysqli_error($con) . "111");
