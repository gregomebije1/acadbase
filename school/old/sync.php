...
if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'Begin Synchronization')) {
  ...
  if (!file_exists("backup.sql")) {
    echo "Local backup file does not exist. Attempting to backup<br>";
	//Then automatically re-run backup_restore.php before syncing..
	backup("backup.sql", $database);
  }
    $source_file = "backup.sql";
  $destination_file = "backup.sql";
...
?>
  <table>
   <tr class='class1'>
	  ...
	  <td><input type="text" name="folder" size="50" value="school"></td>
      </tr>
