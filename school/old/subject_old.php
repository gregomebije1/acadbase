...
if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'confirm_delete')) {
  ...
  $sql="delete from student_subject where subject_id={$_REQUEST['id']} 
    and $extra_caution_sql";
...
<div class='class1'>
 ...
 <?php
  ...
  $class_type = array();
  $sql="select * from class_type where $extra_caution_sql";
  $result = mysqli_query($con, $sql) or die(mysqli_error($con));
 ...
 <?php
  foreach($class_type as $id => $name) {
   ...
   $sql="select * from subject where $extra_caution_sql 
     and class_type_id = $id";
   $result = mysqli_query($con, $sql) or die(mysqli_error($con));
   while($row = mysqli_fetch_array($result)) {
    //echo "<tr><td><a href='subject.php?action=Edit&id={$row['id']}'>{$row['name']}</a></td></tr>";
