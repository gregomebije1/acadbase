   ...
   while($row = mysql_fetch_array($result)) { 
	 echo "
       ...
       <tr>
        <td colspan='2' width='80%' valign='top'>
         <table border='1' valign='top' style='table-layout:row; text-align:center;'>
          <tr>
           <th>Subject</th>
     ...
     ";
      ...
      while ($row1 = mysql_fetch_array($result1)) {
		$total_subject_score = $row1['test']  + $row1['exam'];
        echo "
	   <tr>
        <td style='text-align:left;'>{$row1['name']}</td>
        <td>{$row1['test']}</td>
        <td>{$row1['exam']}</td>
		<td>$total_subject_score</td>
	    <td>" . grade($total_subject_score) . "</td>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
